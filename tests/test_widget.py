from src.widget import mask_account_card, get_dat

import pytest

# Тесты для ф-ии mask_account_card

def test_mask_account_card(num_card_or):
...
def test_mask_account_card(num_acc_or):
    '''Ф-я - тест, которая корректно распознает и применяет нужный тип маскировки'''
...
def test_mask_account_card_invalid(invalid_formate):
    with pytest.raises(ValueError):
        mask_account_card(invalid_formate)


@pytest.mark.parametrize("num_card_or, expected", [
